namespace ArrayApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            int index = Array.IndexOf(arr4, 3); // Returns the index of the first occurrence of the value 3 in the array, or -1 if it is not found
        ...
        public static (long bestSum, int bestRow, int bestCol) FindBestSum(int[,] matrix)
        {
            ...
            for (int i = 0; i < matrix.GetLength(0) - 1; i++)
            {
                ...
                {
                    long sum = matrix[i, j] + matrix[i, j + 1] + matrix[i + 1, j]
                        + matrix[i + 1, j + 1]; // current element and its right, down and diagonal neighbors
